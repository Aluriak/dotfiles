#!/usr/bin/python3
"""Let user pick a commit in current directory, then print it or put it in clipboard

The Picker class and its flotilla are directly copied from the pick package.
...
KEYS_UP = (curses.KEY_UP, ord("s"))
KEYS_DOWN = (curses.KEY_DOWN, ord("t"))
...
def run(number: int = 0, target: str = 'stdout'):
    ps = subprocess.Popen(['git', 'log'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = ps.communicate()
    reg = r'commit ([a-z0-9]+)\nAuthor: ([^<]+)<[^>]+>\nDate:[^\n]+\n\n([^\n]+)\n'
    options = {}
    for match in re.finditer(reg, stdout.decode()):
        cmt, aut, msg = map(str.strip, match.groups())
        options[f"{cmt} ({aut}): {msg}"] = cmt
    pick_options = list(options)[:number] if number else list(options)

    if not pick_options:
        print(f"No commit found. Reason: {stderr.decode() if stderr else 'no commit in that repo'}")
        return
    choice, _ = pick(pick_options, 'select a commit')
...
def parse_cli() -> argparse.Namespace:
    ...
    parser.add_argument('--target', '-t', choices=['clip', 'c', 'stdout', 'stderr', 's', 'e', 'both', 'b'], help='where to put the resulting choice', default='b')
    parser.add_argument('--number', '-n', type=int, help='maximal number of commit to show', default=0)
    parser.add_argument('--path', '-d', type=str, help='path to the git directory', default='.')
